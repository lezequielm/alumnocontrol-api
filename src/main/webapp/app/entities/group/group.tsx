...
export const Group = (props: IGroupProps) => {
  ...
  const { groupList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="group-heading">
        Groups
            ...
            <tbody>
              {groupList.map((group, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{group.name}</td>
                  <td>{group.enabled ? 'true' : 'false'}</td>
                  <td>{group.institute ? <Link to={`institute/${group.institute.id}`}>{group.institute.name}</Link> : ''}</td>
